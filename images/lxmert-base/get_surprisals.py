...
def _get_predictions_inner(sentence, tokenizer, model, device):
    ...
    with torch.no_grad():
        ...
        for t in range(tokens_tensor.shape[0]):
            ...
            out = model(input_ids=tt_copy, labels=labels, visual_feats=vfeats, visual_pos=vpos)
            logits = out[1]
...
def main(args):
    ...
    model = tr.LxmertForPreTraining.from_pretrained("unc-nlp/lxmert-base-uncased")
    total = torch.cuda.get_device_properties(0).total_memory
    res = torch.cuda.memory_reserved(0)
    allo = torch.cuda.memory_allocated(0)
    print(total, res, allo, file=sys.stderr)
